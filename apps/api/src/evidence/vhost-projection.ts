import { FFUF_MAX_JSON_BYTES, isVhostArtifactSlot } from "@stonehush/contracts";
import type { VhostRepository } from "@stonehush/db";
...
export class VhostProjectionService {
  constructor(
    private readonly store: EvidenceStore,
    private readonly repo: VhostRepository,
  ) {}
  ...
  async projectForArtifact(artifactId: string): Promise<ProjectionResult> {
    ...
    if (!isVhostArtifactSlot(row.artifactSlot) || row.kind !== "tool_raw") {
